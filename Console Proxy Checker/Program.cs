...
namespace Console_Proxy_Checker
{
    class Program
    {
        ...
        static void Watermark()
        {
            ...
            Console.WriteLine("  =========== Console Proxy Checker ===========", ConsoleColor.Yellow, ConsoleColor.Black);
        ...
        {
            Console.Title = "Console Proxy Checker, Dev: </Sir.Arash>";
            ...
            else
            {
                foreach (string line in File.ReadAllLines(args[0]))
                {
                    ...
                    Console.Title = $"Console Proxy Checker | Loaded: {Proxies.Count.ToString()} proxies";
                }
        ...
        static void Check()
        {
            for (int i = 0; i < Proxies.Count; i++)
            {
                ...
                if (reply.Status == IPStatus.Success)
                {
                    ...
                    Console.Title = $"Console Proxy Checker | Loaded: {Proxies.Count.ToString()} proxies | Working proxies: {WorkingProxies.Count} | Dead proxies: {NonWorkingProxies.Count}";
                }
                else
                {
                    JObject jobject = JObject.Parse(new WebClient().DownloadString("http://ip-api.com/json/" + ip));
